...
The former `scripts/migrate-package-catalog.ts` is retired because it could create a missing feature and replace package reference arrays. The gated migration is `scripts/migrate-prerna-content.ts`: it resolves and verifies the existing package references, refuses missing/ambiguous feature documents, emits leaf-path changes and deliberate array replacements, is dry-run by default, and requires `--apply --backup-dir <directory>` plus the write token before any transaction. It never creates documents or targets drafts. A timestamped export is written and read back successfully before changed fields are committed; restoring that export is the rollback path.
