...
const BookingContext = createContext<{open: (packageName?: string) => void} | null>(null);
...
export default function BookingProvider({children}: {children: React.ReactNode}) {
 ...
 // The package whose CTA opened the dialog; reported on the form's events.
 ...
 return <BookingContext.Provider value={{open: (name?: string) => {setPackageName(name); setOpen(true);}}}>
  ...
  <div aria-hidden={!open} inert={!open} style={{display:open ? undefined : "none"}} className="fixed inset-0 z-[1000] flex items-end justify-center bg-black/40 p-3 sm:items-center" onClick={() => setOpen(false)}>
   <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby="booking-title" tabIndex={-1} data-lenis-prevent className="mason-assessment max-h-[90svh] w-full max-w-xl overflow-y-auto rounded-3xl bg-sand-50 p-6 sm:p-8" onClick={event => event.stopPropagation()}>
    <div className="mb-6 flex items-start justify-between gap-4"><h2 id="booking-title" className="h-display text-3xl">Book Free <span className="accent-word">Inspection</span></h2><button type="button" className="min-h-11 min-w-11" aria-label="Close booking" onClick={() => setOpen(false)}>✕</button></div>
    <AssessmentLeadForm packageName={packageName} />
   </div>
  </div>
